...
const ChatAssistant: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const updatedChat = {
      ...currentChat,
      date: new Date(), // Update date to put it at the top
    ...
    setTimeout(() => {
      ...
      const finalHistory = updatedHistory.map(chat => 
        chat.id === activeChat ? chatWithResponse : chat
      );
  ...
  const toggleFavorite = (id: number) => {
    const updatedApps = gptApps.map(app => 
      app.id === id ? { ...app, isFavorite: !app.isFavorite } : app
    );
    setGptApps(updatedApps);
    
    toast({
      title: updatedApps.find(app => app.id === id)?.isFavorite 
        ? "Added to favorites" 
                    ...
                    <div className="p-4 space-y-4">
                      {gptApps.filter(app => app.isFavorite).length === 0 ? (
                        ...
                        gptApps
                          ...
                          .map(app => (
                            <Card key={app.id} className="cursor-pointer hover:bg-accent/50 transition-colors">
                      ...
                      <>
                        {chatHistory.find(chat => chat.id === activeChat)?.messages.length === 0 ? (
                          ...
                          <div className="space-y-4">
                            {chatHistory.find(chat => chat.id === activeChat)?.messages.map(msg => (
                                          ...
                                          {msg.attachments.map((attachment, index) => (
                                            <div key={index}>
                                              {attachment.type === 'image' ? (
                                                ...
                                                <div className="bg-background/80 text-foreground rounded p-2 overflow-x-auto mb-2">
                                                ...
                                                <div className="bg-background/80 text-foreground rounded p-2 flex items-center gap-2">
                  ...
                  <div className="border-t py-4 sticky bottom-0 bg-background">
                    <form onSubmit={handleSubmit} className="flex flex-col space-y-2">
                      <div className="flex items-center space-x-2">
                        ...
                        {selectedFile && (
                          <div className="flex items-center gap-2 bg-secondary/50 py-1 px-2 rounded-md">
              ...
              <TabsContent value="gpt" className="flex-1 flex flex-col data-[state=active]:flex-1">
                <div className="max-w-4xl w-full mx-auto px-4 py-6 flex flex-col h-full">
                  ...
                  <div className="mb-6 space-y-4">
                    ...
                    <ScrollArea className="whitespace-nowrap pb-3">
                      <div className="flex gap-2">
                        ...
                        {Array.from(new Set(gptApps.map(app => app.category))).map(category => (
                          <Button
                            key={category}
                  ...
                  {filteredGptApps.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 overflow-y-auto pb-4">
